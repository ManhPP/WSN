...
def solve_by_pulp(inp, is_adj_matrix, distance_matrix, dict_constant):
    ...
    for k in range(1, num_all_vertex):
        for j in range(1, num_all_vertex):
            if  j != k:
                lp_prob +=pulp.lpSum(y[i, j, k] for i in range(num_all_vertex)) - pulp.lpSum(y[j, i, k] for i in range(1, num_all_vertex)) == 0
    
    for j in range(1, num_all_vertex):
        lp_prob +=pulp.lpSum(y[i, j, j] for i in range(num_all_vertex)) == e[j]

    for k in range(1, num_all_vertex):
        lp_prob +=pulp.lpSum(y[i, j, k] for i in range(num_all_vertex) for j in range(1, num_all_vertex)) <= inp.max_hop
